...
function samplePoisson(lambda) {
  ...
  if (lambda > 80) {
    const normal = Math.sqrt(lambda) * gaussianRandom() + lambda;
...
function runSelfTests() {
  ...
  const state = { rise: 1, fall: 1, lastTime: 0 };
  const tauRise = 0.01;
  const tauFall = 0.06;
  advanceAnalogState(state, 0.02, tauRise, tauFall);
  console.assert(
    state.rise < 1 && state.fall < 1 && state.lastTime === 0.02,
    "Analog state should decay forward in time",
  );
  const prevRise = state.rise;
  advanceAnalogState(state, 0.5, tauRise, tauFall);
  console.assert(state.rise < prevRise && state.fall < prevRise, "Analog state should continue decaying");

  const blend0 = 0;
  const blendTarget = 1;
  const dt = 0.15;
  const blend1 = stepTowards(blend0, blendTarget, dt, 0.15);
  console.assert(blend1 > blend0 && blend1 < blendTarget, "Blend step should move toward target");
}
