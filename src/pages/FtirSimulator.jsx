...
function buildSourceMix({
  halogenMag, halogenTempK = 3200,
  laserMag, laserNm, laserWidth, absorptionMedia,
  ...
  for (let nm = SOURCE_MIN_NM; nm <= SOURCE_MAX_NM; nm += step) {
    ...
    if (halogenMag > 0) {
      const bb = blackbodyRel(nm, Math.max(1500, Math.min(5000, halogenTempK || 3200)));
      val += halogenMag * bb;
    }
...
export default function FTIR_Michelson_VCSEL_Sim() {
  ...
  const [halogenTempK, setHalogenTempK] = useState(3200);
                        ...
                        <div className="space-y-1 rounded-md border border-slate-200 p-3">
                          ...
                          <div className="pt-3 space-y-1">
                            <div className="flex items-center justify-between text-sm text-slate-600">
                              ...
                              <span className="tabular-nums">{fmt(halogenTempK,0)} K</span>
                            </div>
                            <Slider
                              value={[halogenTempK]}
                              min={2400}
                              max={3600}
                              step={25}
                              onValueChange={([v]) => setHalogenTempK(v)}
                            />
                          </div>
                        </div>
                        {/* Xenon arc */}
