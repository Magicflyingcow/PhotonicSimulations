...
export function Slider({ value = [0], min = 0, max = 100, step = 1, onValueChange, className }) {
  ...
  if (value.length === 1) {
    return (
      <input
        ...
        type="range"
        className={cn(
          "h-2 w-full cursor-pointer appearance-none rounded-full bg-slate-800 accent-sky-500 [&::-webkit-slider-thumb]:h-4 [&::-webkit-slider-thumb]:w-4 [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-sky-400 [&::-moz-range-thumb]:h-4 [&::-moz-range-thumb]:w-4 [&::-moz-range-thumb]:rounded-full [&::-moz-range-thumb]:border-0 [&::-moz-range-thumb]:bg-sky-400",
          className
        )}
        min={min}
        max={max}
  ...
  return (
    <div className={cn("relative flex w-full items-center", className)}>
      <div className="absolute left-0 right-0 h-1 rounded-full bg-slate-800" />
      <div
        className="absolute h-1 rounded-full bg-sky-400"
        style={{
